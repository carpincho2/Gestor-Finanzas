...
Hemos simplificado la carpeta `mds/` dejando exclusivamente archivos de documentación en Markdown (.md). A continuación, tienes el mapa de los **7 archivos fundamentales** para el proyecto:
...
| 🔐 [autenticacion_y_responsive.md](autenticacion_y_responsive.md) | Detalle de la implementación segura de Google Sign-In en el backend y el sistema adaptable unificado (CSS media queries, menú hamburguesa). | Para aprender sobre flujos OAuth2 seguros en backend y diseño responsive avanzado. |
...
> [!NOTE]
> Los recursos de configuración y pruebas han sido reorganizados a carpetas semánticas dedicadas:
> * El diccionario de marcas y patrones de OCR está en [data/OCR_PATTERNS.json](../data/OCR_PATTERNS.json).
> * Los ejemplos de implementación de JavaScript están en [tests/OCR_IMPLEMENTATION_EXAMPLES.js](../tests/OCR_IMPLEMENTATION_EXAMPLES.js).
> * Los casos de prueba automatizados están en [tests/OCR_TEST_CASES.js](../tests/OCR_TEST_CASES.js).
> * Los tickets para pruebas manuales están en la carpeta [tests/tickets/](../tests/tickets/).
...
2. Copia y adapta el código de validaciones de [tests/OCR_IMPLEMENTATION_EXAMPLES.js](../tests/OCR_IMPLEMENTATION_EXAMPLES.js) en tu archivo [js/app.js](../js/app.js).
3. Prueba la suite de validación copiando y pegando el código de [tests/OCR_TEST_CASES.js](../tests/OCR_TEST_CASES.js) en la consola (DevTools) y ejecutando `runOCRTestSuite()`.
...
*   **Separación de Responsabilidades**: Por qué decidimos separar la autenticación (`index.html` + `js/auth.js`) de la pantalla interna del dashboard (`main.html` + `js/app.js`) y por qué organizamos los datos en `/data` y las pruebas en `/tests`. (Ver [organizacion.md](organizacion.md)).
...
v3.0 | Junio de 2026 | Estructura de Proyecto Reorganizada y Limpia
